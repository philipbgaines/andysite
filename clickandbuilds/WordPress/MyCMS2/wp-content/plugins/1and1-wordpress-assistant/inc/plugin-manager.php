...
class One_And_One_Plugin_Manager {
	...
	public function get_plugin_manager( $site_type, $theme_id ) {
		...
		$plugin_sections = One_And_One_Sitetype_Filter::get_filtered_category_plugins( $plugins, $site_type );
		?>
		...
		<form action="<?php echo esc_url( add_query_arg( array( 'setup_action' => 'activate' ) ) ); ?>" method="post">
			...
			<div class="wrap">
				...
				<div class="oneandone-functionality-choice">
					...
					<?php if ( ! empty( $plugin_sections ) ) {
						$sections_count = count( $plugin_sections );
						$counter = 0;
						$index = 0;

						foreach ( $plugin_sections as $category_key => $category ) {
							$counter ++;
							...
							<div class="oneandone-plugin-browser">
								...
								foreach ( $category['plugins'] as $plugin ) {
									$this->get_plugin_item( $plugin, $category['options'] );
								}

								if ( $counter == $sections_count ) {
									$this->install_other();
								} ?>
							</div>
	...
	protected function get_plugin_item( $plugin, $options = array() ) {
		...
		$is_selected = ( isset( $options['selected'] ) && ( $options['selected'] == true ) );
		$is_mandatory = ( isset( $options['mandatory'] ) && ( $options['mandatory'] == true ) );
		...
		if ( get_option( 'oneandone_assistant_completed' ) == true ) {
			...
			$is_active = $is_selected || $is_mandatory;
		}
			...
			<div class="plugin-card-top">
				...
				<div class="desc column-description" style="margin-left: 118px;">
					...
					<ul class="plugin-action-buttons">
						<li class="oneandone-install-checkbox<?php echo $is_active ? ' checked' : ''; ?><?php echo $is_mandatory ? ' mandatory' : ''; ?>">
							<label for="plugin-<?php echo $plugin->slug; ?>">

								<?php if ( $is_mandatory ): ?>
									<input id="plugin-<?php echo $plugin->slug; ?>" name="plugins[]"
										   value="<?php echo $plugin->slug; ?>" type="hidden">
									<input type="checkbox" checked disabled>

								<?php else: ?>
									<input id="plugin-<?php echo $plugin->slug; ?>" name="plugins[]"
										   value="<?php echo $plugin->slug; ?>"
										   type="checkbox" <?php echo $is_active ? ' checked' : ''; ?>>
