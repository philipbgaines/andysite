...
class One_And_One_Sitetype_Filter {
	...
	public static function get_filtered_category_plugins( $plugins, $sitetype ) {
		...
		if ( ! empty( $config->sitetypes->$sitetype->plugin_categories ) ) {
			...
			foreach ( $config->sitetypes->$sitetype->plugin_categories as $category => $options ) {
				$filtered_plugins[$category]['options'] = ( array ) $options;
				$filtered_plugins[$category]['plugins'] = array();

				self::load_plugins(
					self::get_plugins_slugs( $sitetype, array( $category ) ),
					$filtered_plugins[$category]['plugins'],
					$plugins
				);
			}
		}
