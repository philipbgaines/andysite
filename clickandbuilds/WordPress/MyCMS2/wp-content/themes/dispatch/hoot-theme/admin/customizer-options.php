...
function hoot_theme_customizer_options() {
	...
	if ( $hoot_customizer_load_premiumsection ) :
			...
			if ( !empty( $hoot_options_premium ) && is_array( $hoot_options_premium ) ):
				foreach ( $hoot_options_premium as $pkey => $pfeature ) :
					if ( !empty( $pfeature['type'] ) && ( $pfeature['type'] == 'bottom' || $pfeature['type'] == 'top' ) ) {
						...
						if ( !empty( $plink ) ) $pcontent .= '<a class="button button-primary" href="' . $plink . '" target="_blank"><i class="fa fa-star"></i> ' . __( 'Buy Now', 'dispatch' ) . '</a>';
						if ( !empty( $pdemo ) ) $pcontent .= '<a class="button button-secondary" href="' . $pdemo . '" target="_blank"><i class="fa fa-eye"></i> ' . __( 'Theme Demo', 'dispatch' ) . '</a>';
						$pcontent .= '</div>';
		...
		$settings['premium-use'] = array(
			// 'label'       => __( 'Premium Options', 'dispatch' ),
			'section'     => $section,
			'type'        => 'content',
			'priority'    => '505', // Non static options must have a priority
			'content'     => $pcontent,
		);
