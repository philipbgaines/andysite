 <?php
/*
 Template Name: Contained OnePage Section
 * @package:			WordPress
...
<script>
	jQuery(document).ready(function() {
		...
		jQuery('.section-page').css('min-height',windowHeight).css('padding-top','<?php echo $hgr_page_top_padding+60; ?>px');
	})
</script>
<?php endif; ?>
 
 <div id="<?php echo $post->post_name;?>" class="row section-page <?php echo $parallaxClass;?> <?php echo $hgr_page_color_scheme;?>"  style=" <?php echo $parallaxImageUrl;?> <?php echo $backgroundColor;?> <?php echo ( !empty($hgr_page_height) ? ' height:'.$hgr_page_height.'px!important; ' : '');?> <?php echo ( !empty($hgr_page_top_padding) ? ' padding-top:'.$hgr_page_top_padding.'px!important;' : '' );?> <?php echo ( !empty($hgr_page_btm_padding) ? ' padding-bottom:'.$hgr_page_btm_padding.'px!important;' : '' );?> ">
  <div class="col-md-12" >
    <div class="container">
